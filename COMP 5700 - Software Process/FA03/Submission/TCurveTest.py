import unittest
import FA03.prod.TCurve as TCurve
import math
...
#original test case
    def test600_610_ShouldWorkWithHardCodedSimpsons(self):
        ...
        h = 5
        myT = TCurve.TCurve(n)
        self.assertAlmostEquals(25.0, myT.integrate(f, n, l, h))

# Happy path
    def test600_020ShouldCalculateNominalCase(self):
        def f(x, n):
            return 5
        n = 5
        l = 0
        h = 1
        myT = TCurve.TCurve(n)
        self.assertAlmostEquals(5.0, myT.integrate(f, n, l, h))
        
    def test600_030ShouldCalculateNominalCase(self):
        def f(x, n):
            return 5**2
        n = 5
        l = 0
    ...
    def test600_040ShouldCalculateNominalCase(self):
        def f(x, n):
            return 5**6
        n = 5
        l = 0
        h = 1
        myT = TCurve.TCurve(n)
        self.assertAlmostEquals(15625.0, myT.integrate(f, n, l, h))
        
    def test600_050ShouldCalculateNominalCase(self):
        def f(x, n):
            return 5**100
        n = 5
        l = 0
        h = 1
        myT = TCurve.TCurve(n)
        self.assertAlmostEquals(7.888609052210117e+69, myT.integrate(f, n, l, h))
                
# Sad path
    #Parameters and Values are already checked prior to getting to integrate() 
