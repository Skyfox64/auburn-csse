...
public:
	...
	vector<Node> shortestPath;		//The updated shortest path
	vector<Node> markedPoints;		//Visited nodes
};
...
void System::runProgram(){
	...
	while (true) {
		...
		//If the filename.txt exists
			...
			delete[] mazeSolver.nodeArray;
			mazeSolver.nodeArray = NULL;
		}
		else {
...
void BFSMazeSolver::constructMaze(){
	...
	for (int i = 0; i < numOfNodes; i++){
		...
		//	j++;
		//	newNode.setNumberUsed(j); //This is used as a size of an array
		//}		
		...
		while (inStringStream >> attachedNodeNameIn) {
			Node *attachedNode = new Node(attachedNodeNameIn);
			newNode->attachNewNode(attachedNode, index);
			newNode->setNumberUsed(index++);
		}
